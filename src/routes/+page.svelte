...
<div class="time-entries-container">
	<h1 class="time-entries-header">Anwesenheitszeiten</h1>

	{#if data.anwesenheitszeiten && data.anwesenheitszeiten.length > 0}
		<div class="time-entries-table-wrapper">
			<table class="time-entries-table">
				<thead>
					<tr>
						{#each Object.keys(data.anwesenheitszeiten[0]) as header}
				...
				<tbody>
					{#each data.anwesenheitszeiten as entry}
						<tr>
							{#each Object.entries(entry) as [key, value]}
								<td class="time-entries-table-td">
									{#if (key === 'Kommt' || key === 'Geht') && typeof value === 'string'}
